...
<div
    ...
    <h5 class="mb-3 text-lg font-semibold text-[#9D5425]">
        Il y a actuellement {{count($categories)}} catégories
    </h5>
    </div>
</div>
@endif
    ...
    @foreach ($categories as $category)
        <div class="justify-center flex-row p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700 flex flex-wrap gap-6">
            ...
            <div class="flex flex-wrap gap-5 mt-3 mb-3">
                <a href="{{route('admin-category-edit', $category->id)}}" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-500 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Modifier
                </a>
                <form action="{{ route('admin-category-delete', $category->id) }}" method="post" class="inline">
                <a href="{{route('admin-category-delete', $category->id)}}" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Supprimer
                </a>
                </form>
                <a href="{{ route('admin-category-show', $category->id)}}" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-green-500 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Voir
                </a>
            </div>
        </div>
